...
public class BenchmarkAppTest extends AbstractBenchmarkTest {
    ...
    public void testSelectAndUpdateBenchmarkRuns() throws Exception {
        ...
        waitForRequest(r -> r.getSql().contains("UPDATE my_table") || r.getSql().contains("INSERT INTO my_table"));
        ...
        Thread thread = new Thread(() -> {
            try {
                new picocli.CommandLine(new BenchmarkApp()).execute(new String[]{
                        ...
                        "point-select", "-t", "my_table", "--tps", "5000", "--threads", "10",
                        "--load-type", "SPIKY",
        ...
        Thread thread = new Thread(() -> {
            try {
                new picocli.CommandLine(new BenchmarkApp()).execute(new String[]{
                        "-p", "my-project", "-i", "my-instance", "-d", "my-database",
                        "--host", "http://localhost:" + port,
                        "read-large-result-set", "-t", "my_table", "--num-rows", "10", "--tps", "1000", "--threads", "5"
                });
            } catch (Exception e) {
                System.out.println("App terminated: " + e.getMessage());
            }
    ...
    public void testTpccBenchmarkRuns() throws Exception {
        Thread thread = new Thread(() -> {
            try {
                new picocli.CommandLine(new BenchmarkApp()).execute(new String[]{
                        "-p", "my-project", "-i", "my-instance", "-d", "my-database",
                        "--duration", "2s", "--host", "http://localhost:" + port,
                        "tpcc", "--warehouses", "1", "--clients", "2", "--items", "100"
                });
            } catch (Exception e) {
                System.out.println("App terminated: " + e.getMessage());
            }
    ...
    public void testMetricsCollection() throws Exception {
        Thread thread = new Thread(() -> {
            try {
                new picocli.CommandLine(new BenchmarkApp()).execute(new String[]{
                        "-p", "my-project", "-i", "my-instance", "-d", "my-database",
                        "--host", "http://localhost:" + port,
                        "--resource-probe-interval", "1s",
                        "point-select", "-t", "my_table", "--tps", "1000", "--threads", "2"
                });
            } catch (Exception e) {
                System.out.println("App terminated: " + e.getMessage());
            }
        ...
        while (System.currentTimeMillis() - startTime < 6000) {
            ...
            io.opentelemetry.sdk.metrics.data.MetricData opCount = metricData.stream()
                    .filter(md -> md.getName().equals(BenchmarkApp.OPERATION_COUNT_NAME))
                    .findFirst().orElse(null);
            io.opentelemetry.sdk.metrics.data.MetricData mem = metricData.stream()
                    .filter(md -> md.getName().equals(BenchmarkApp.MEMORY_USAGE_NAME))
                    .findFirst().orElse(null);
            io.opentelemetry.sdk.metrics.data.MetricData cpu = metricData.stream()
            ...
            if (opCount != null && !opCount.getData().getPoints().isEmpty()) {
                io.opentelemetry.sdk.metrics.data.PointData firstPoint = (io.opentelemetry.sdk.metrics.data.PointData) opCount.getData().getPoints().iterator().next();
                String client = firstPoint.getAttributes().get(io.opentelemetry.api.common.AttributeKey.stringKey("client"));
                String benchmarkType = firstPoint.getAttributes().get(io.opentelemetry.api.common.AttributeKey.stringKey("benchmark_type"));

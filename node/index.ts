...
async function main() {
  ...
  program
    ...
    .option(
      '--workers <workers>',
      'Number of parallel worker threads to execute the workload across. Defaults to min(threads, available client CPUs).',
    )
...
async function runBenchmarkAction(
  ...
  const threads =
    type === 'tpcc'
      ? parseInt(subOpts.clients || '10', 10)
      : subOpts.threads
  ...
  const cpuCount = process.env.BENCHMARK_CPU_LIMIT
    ...
    : os.availableParallelism
      ? os.availableParallelism()
      : os.cpus().length;
  const maxAvailableWorkers =
    process.env.USE_SIDECAR === 'true'
      ? Math.max(1, Math.floor(cpuCount) - 1)
      : Math.max(1, Math.floor(cpuCount));
  const autoWorkers = Math.max(1, Math.min(threads, maxAvailableWorkers));
  const workersStr =
    subOpts.workers ?? globalOpts.workers ?? process.env.WORKERS;
  const workers = workersStr ? parseInt(workersStr, 10) : autoWorkers;

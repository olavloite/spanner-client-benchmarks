...
export async function executeNewOrder(database: Database, scaleFactor: number, totalItems: number): Promise<void> {
  ...
  await database.runTransactionAsync(async (transaction) => {
    ...
    const batchStatements: any[] = [
      ...
      },
      {
        ...
        params: { w: warehouseId, d: districtId, o: nextOrderId, c: customerId, dt: now, cnt: numItems },
        types: { w: "int64", d: "int64", o: "int64", c: "int64", dt: "timestamp", cnt: "int64" },
      },
      {
    ...
    for (let i = 0; i < numItems; i++) {
      batchStatements.push({
        ...
        params: { w: warehouseId, d: districtId, o: nextOrderId, ol: i + 1, i: itemIds[i], qty: quantities[i], amt: Spanner.float(25.0) },
        types: { w: "int64", d: "int64", o: "int64", ol: "int64", i: "int64", qty: "int64", amt: "float64" },
      });
      batchStatements.push({
...
export async function executePayment(database: Database, scaleFactor: number): Promise<void> {
  const warehouseId = Math.floor(Math.random() * scaleFactor) + 1;
  const districtId = Math.floor(Math.random() * 10) + 1;
  const customerId = Math.floor(Math.random() * 3000) + 1;
  ...
  await database.runTransactionAsync(async (transaction) => {
    const stmts = [
      ...
      },
      {
        ...
        params: { amt: Spanner.float(amount), w: warehouseId, d: districtId, c: customerId },
      ...
      },
      {
        ...
        params: { w: warehouseId, d: districtId, h: uuidv4(), c: customerId, dt: new Date(), amt: Spanner.float(amount) },
        types: { w: "int64", d: "int64", h: "string", c: "int64", dt: "timestamp", amt: "float64" },
      },
    ];
...
export async function executeOrderStatus(database: Database, scaleFactor: number): Promise<void> {
  const warehouseId = Math.floor(Math.random() * scaleFactor) + 1;
  const districtId = Math.floor(Math.random() * 10) + 1;
  const customerId = Math.floor(Math.random() * 3000) + 1;
...
export async function executeDelivery(database: Database, scaleFactor: number): Promise<void> {
  const warehouseId = Math.floor(Math.random() * scaleFactor) + 1;
...
export async function executeStockLevel(database: Database, scaleFactor: number): Promise<void> {
  const warehouseId = Math.floor(Math.random() * scaleFactor) + 1;
  const districtId = Math.floor(Math.random() * 10) + 1;
  ...
  try {
    ...
    if (distRows.length > 0) {
      ...
      const stockQuery = {
        ...
        params: { w: warehouseId, d: districtId, minOrderId, nextOrderId, threshold },
        types: { w: "int64", d: "int64", minOrderId: "int64", nextOrderId: "int64", threshold: "int64" },
      };

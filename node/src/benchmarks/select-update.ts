 ...
 */
export async function executeSelectAndUpdate(
  database: Database,
  tableName: string,
  minId: number,
  maxId: number,
  ...
  await database.runTransactionAsync(async transaction => {
    ...
    const randomValue = generateRandomString(randomStrLength);
...
export class SelectAndUpdateBenchmark extends AbstractBenchmark {
  ...
  public async execute(
    ...
    return executeSelectAndUpdate(database, tableName, minId, maxId);
  }
}

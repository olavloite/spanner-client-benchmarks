...
if [ "$USE_RELEASED_VERSION" = "false" ]; then
  echo "Building local Spanner client package from copied source..."
  cd ./spanner-src
  rm -f package-lock.json
  npm install
  COMPILE_RUST_ADDON="$COMPILE_RUST_ADDON" COMPILE_GO_ADDON="$COMPILE_GO_ADDON" npm run compile
  ...
  # Setup cleanup trap to restore files on exit
  trap 'echo "Restoring backups..."; cd "$INIT_DIR"; [ -f package.json.bak ] && mv package.json.bak package.json; [ -f package-lock.json.bak ] && mv package-lock.json.bak package-lock.json' EXIT

  echo "Installing local Spanner client in benchmark application..."
  npm install "./spanner-src/$TARBALL_NAME" --legacy-peer-deps
else

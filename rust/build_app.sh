...
if [ "$USE_RELEASED_VERSION" = "false" ]; then
  ...
  if [ "$CLIENT_BRANCH" != "main" ]; then
    if [[ "$CLIENT_BRANCH" =~ ^[0-9a-f]{7,40}$ ]]; then
      ...
      sed -i.bak "s|git = \"$CLIENT_REPO\", package = |git = \"$CLIENT_REPO\", rev = \"$CLIENT_BRANCH\", package = |g" Cargo.toml
    else
      echo "Updating Cargo.toml to use branch $CLIENT_BRANCH..."
      sed -i.bak "s|git = \"$CLIENT_REPO\", package = |git = \"$CLIENT_REPO\", branch = \"$CLIENT_BRANCH\", package = |g" Cargo.toml
    fi
  fi
  rm -f Cargo.toml.bak
fi

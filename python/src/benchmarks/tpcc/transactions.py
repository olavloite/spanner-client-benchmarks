...
def execute_new_order(database: Database, scale_factor: int, total_items: int) -> None:
    ...
    def _cb(transaction: Transaction):
        results = transaction.execute_sql(
            ...
            param_types={"w": spanner.param_types.INT64, "d": spanner.param_types.INT64},
        )
        next_order_id = 1000
        for row in results:
            next_order_id = row[0]
        ...
        cust_results = transaction.execute_sql(
            ...
            param_types={"w": spanner.param_types.INT64, "d": spanner.param_types.INT64, "c": spanner.param_types.INT64},
        )
        ...
        batch_statements = [
            (
                ...
                {"next": spanner.param_types.INT64, "w": spanner.param_types.INT64, "d": spanner.param_types.INT64}
            ),
            (
                "INSERT INTO orders (warehouse_id, district_id, order_id, customer_id, entry_date, item_count, all_local) VALUES (@w, @d, @o, @c, @dt, @cnt, 1)",
                {"w": warehouse_id, "d": district_id, "o": next_order_id, "c": customer_id, "dt": now, "cnt": num_items},
                {"w": spanner.param_types.INT64, "d": spanner.param_types.INT64, "o": spanner.param_types.INT64, "c": spanner.param_types.INT64, "dt": spanner.param_types.TIMESTAMP, "cnt": spanner.param_types.INT64}
            ),
            (
                ...
                {"w": spanner.param_types.INT64, "d": spanner.param_types.INT64, "o": spanner.param_types.INT64, "dt": spanner.param_types.TIMESTAMP}
            ),
        ]
        ...
        for i in range(num_items):
            batch_statements.append((
                ...
                {"w": warehouse_id, "d": district_id, "o": next_order_id, "ol": i + 1, "i": item_ids[i], "qty": quantities[i], "amt": 25.0},
                {"w": spanner.param_types.INT64, "d": spanner.param_types.INT64, "o": spanner.param_types.INT64, "ol": spanner.param_types.INT64, "i": spanner.param_types.INT64, "qty": spanner.param_types.INT64, "amt": spanner.param_types.FLOAT64}
            ))
            batch_statements.append((
                ...
                {"qty": spanner.param_types.INT64, "w": spanner.param_types.INT64, "i": spanner.param_types.INT64}
            ))
    ...
    def _cb(transaction: Transaction):
        batch_statements = [
            ...
            (
                ...
                {"amt": spanner.param_types.FLOAT64, "w": spanner.param_types.INT64, "d": spanner.param_types.INT64}
            ),
            (
                ...
                {"amt": spanner.param_types.FLOAT64, "w": spanner.param_types.INT64, "d": spanner.param_types.INT64, "c": spanner.param_types.INT64}
            ),
            (
                "INSERT INTO history (warehouse_id, district_id, history_id, customer_id, date, amount, data) VALUES (@w, @d, @h, @c, @dt, @amt, 'history')",
                {"w": warehouse_id, "d": district_id, "h": str(uuid.uuid4()), "c": customer_id, "dt": datetime.datetime.now(datetime.timezone.utc), "amt": amount},
                {"w": spanner.param_types.INT64, "d": spanner.param_types.INT64, "h": spanner.param_types.STRING, "c": spanner.param_types.INT64, "dt": spanner.param_types.TIMESTAMP, "amt": spanner.param_types.FLOAT64}
            ),
        ]
        transaction.batch_update(batch_statements)
    ...
    with database.snapshot(multi_use=True) as snapshot:
        ...
        if order_id != -1:
            line_res = snapshot.execute_sql(
                ...
                param_types={"w": spanner.param_types.INT64, "d": spanner.param_types.INT64, "o": spanner.param_types.INT64},
            )
        ...
        for district_id in range(1, 11):
            ...
            if order_id != -1:
                batch_statements.append((
                    ...
                    {"w": spanner.param_types.INT64, "d": spanner.param_types.INT64, "o": spanner.param_types.INT64}
                ))
                batch_statements.append((
                    "UPDATE orders SET carrier_id = @c WHERE warehouse_id = @w AND district_id = @d AND order_id = @o",
                    {"c": carrier_id, "w": warehouse_id, "d": district_id, "o": order_id},
                    {"c": spanner.param_types.INT64, "w": spanner.param_types.INT64, "d": spanner.param_types.INT64, "o": spanner.param_types.INT64}
                ))
                batch_statements.append((
                    "UPDATE order_line SET delivery_date = @dt WHERE warehouse_id = @w AND district_id = @d AND order_id = @o",
                    {"dt": datetime.datetime.now(datetime.timezone.utc), "w": warehouse_id, "d": district_id, "o": order_id},
                    {"dt": spanner.param_types.TIMESTAMP, "w": spanner.param_types.INT64, "d": spanner.param_types.INT64, "o": spanner.param_types.INT64}
                ))
        if batch_statements:
            transaction.batch_update(batch_statements)
    ...
    with database.snapshot(multi_use=True) as snapshot:
        ...
        if next_order_id != -1:
            ...
            stock_res = snapshot.execute_sql(
                ...
                params={"w": warehouse_id, "d": district_id, "min_order_id": min_order_id, "next_order_id": next_order_id, "threshold": threshold},
                param_types={"w": spanner.param_types.INT64, "d": spanner.param_types.INT64, "min_order_id": spanner.param_types.INT64, "next_order_id": spanner.param_types.INT64, "threshold": spanner.param_types.INT64},
            )
            for _ in stock_res:
                pass
